...
class AivoovTtsService {
    ...
    private processTextForTts(text: string): string {
        ...
        console.log(
            '[Aivoov TTS] After markdown conversion:',
            speechText.length,
            'chars'
        )
        console.log(
            '[Aivoov TTS] Converted text sample:',
            speechText.substring(0, 150) + '...'
        )
        ...
        console.log(
            '[Aivoov TTS] Final text (first 100 chars):',
            result.substring(0, 100) + '...'
        )
        console.log(
            '[Aivoov TTS] Text processed with proper pauses for headings and lists'
        )
    ...
    public async generateSpeech(
        ...
        if (this.cache.has(cacheKey)) {
            ...
            if (this.isCacheValid(cachedAudio) && cachedAudio.url) {
                console.log(
                    '[Aivoov TTS] Using valid cached audio URL - no API call needed'
                )
                console.log(
                    '[Aivoov TTS] Creating fresh audio element from cached URL:',
                    cachedAudio.url
                )
                ...
                console.log(
                    '[Aivoov TTS] Fresh audio element created from cache'
                )
                return audio
            } else {
                console.log(
                    '[Aivoov TTS] Cache expired or invalid, removing from cache'
                )
        ...
        try {
            ...
            console.log(
                '[Aivoov TTS] Full response object:',
            ...
            console.log(
                '[Aivoov TTS] Audio field length:',
                result.audio ? result.audio.length : 'null/undefined'
            )
            ...
            console.log(
                '[Aivoov TTS] Audio blob created, size:',
                audioBlob.size,
                'bytes'
            )
            ...
            console.log(
                '[Aivoov TTS] Audio URL cached successfully with key:',
                cacheKey
            )
            console.log(
                '[Aivoov TTS] Cache now contains',
                this.cache.size,
                'items'
            )
            console.log(
                '[Aivoov TTS] Note: Only URL cached, fresh audio elements created each time'
            )
            ...
            if (axios.isAxiosError(error)) {
                ...
                if (error.response?.status === 403) {
                    throw new Error(
                        'Aivoov TTS API access forbidden. Check your API key.'
                    )
                }
                if (error.response?.status && error.response.status >= 500) {
                    throw new Error(
                        'Aivoov TTS server error. Please try again later.'
                    )
                }
            }
