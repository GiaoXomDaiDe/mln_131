...
NHỚ: Bạn là chuyên gia về Tư tưởng Hồ Chí Minh, KHÔNG phải AI tổng quát!`
    ...
    private normalizeVietnameseText(text: string): string {
        return text
            .toLowerCase()
            .replace(/à|á|ạ|ả|ã|â|ầ|ấ|ậh|ẩ|ẫ|ă|ằ|ắp|ặ|ẳ|ẵ/g, 'a')
            .replace(/è|é|ẹ|ẻ|ẽ|ê|ề|ế|ệ|ể|ễ/g, 'e')
            .replace(/ì|í|ị|ỉ|ĩ/g, 'i')
            .replace(/ò|ó|ọ|ỏ|õ|ô|ồ|ố|ộ|ổ|ỗ|ơ|ờh|ởf|ợ|ở|ỡ/g, 'o')
            .replace(/ù|ú|ụ|ủ|ũ|ư|ừ|ứ|ự|ữ|ự/g, 'u')
            .replace(/ỳ|ý|ỵ|ỷ|ỹ/g, 'y')
            .replace(/đ/g, 'd')
    }

    /**
     * Phản hồi từ chối NGHIÊN NGặT
     */
    private createRejectionResponse(): string {
        return `⛔ **Câu hỏi ngoài chuyên môn của tôi**

� Tôi là **AI Gia Sư Tư Tưởng Hồ Chí Minh**, chỉ trả lời các câu hỏi về:

✅ **CHUỴ ĐỀ TÔI GIÚP BẠN:**
• Tư tưởng Hồ Chí Minh về đại đoàn kết toàn dân tộc
• Vai trò chiến lược của đại đoàn kết
• Lực lượng và nền tảng của khối đại đoàn kết
• Điều kiện xây dựng đại đoàn kết
• Mặt trận dân tộc thống nhất
• Phương thức xây dựng khối đại đoàn kết

⛔ **TÔI KHÔNG TRẢ LỜI về:**
• Anime, manga, game, giải trí
• Thể thao, bóng đá, nhân vật nổi tiếng
• Công nghệ, điện thoại, lập trình
• Đời sống cá nhân, tình yêu, sức khỏe
• Chính trị quốc tế (ngoài Việt Nam)
• Bất kỳ chủ đề khác không liên quan

� **VÍ DỤ CÂU HỎI PHÙ HỢP:**
• "Vai trò chiến lược của đại đoàn kết là gì?"
• "Nền tảng của khối đại đoàn kết theo Hồ Chí Minh?"
• "Điều kiện để xây dựng đại đoàn kết là gì?"

� **Hãy hỏi về Tư tưởng Hồ Chí Minh nhé!**`
    }
...
// const tutor = new AIPersonalTutor(apiKey)
// const response = await tutor.chatWithTutor('user123', 'Giải thích khái niệm CNXH là gì?', progress, enhancedContext)
// const suggestions = await tutor.suggestNextLesson(progress)
// const analysis = await tutor.analyzeWeaknesses(progress)
